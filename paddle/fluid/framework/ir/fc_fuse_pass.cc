...
int FCFusePass::ApplyFCPattern(Graph* graph, bool with_relu) const {
  ...
  auto handler = [&](const GraphPatternDetector::subgraph_t& subgraph,
    ...
    if ((mul_op_desc->HasAttr("quantize_weight_bits"))) {
      desc.SetAttr("quantize_weight_bits", mul_op_desc->GetAttr("quantize_weight_bits"));
      auto scale_name = w->Name() + "_quant_scale";
      VLOG(3) << "get scale: " << scale_name;
      PADDLE_ENFORCE_EQ(
        mul_op_desc->HasAttr(scale_name), true,
        platform::errors::NotFound(
            "scale(%s) not found in FCFuse pass.",
            scale_name));
      desc.SetAttr("weight_scale", mul_op_desc->GetAttr(scale_name));
    }
