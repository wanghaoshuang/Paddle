...
#include "paddle/fluid/framework/ir/transformer_qkv_fc_fuse_pass.h"
...
int TansformerQKVProjectionFusePass::BuildFusion(Graph* graph,
  ...
  auto fuse_creater = [&](
      ...
      Node* eltadd0_out, Node* eltadd1_out, Node* eltadd2_out // bias outputs
      ) {
    ...
    fused_qkv_op_desc.SetOutput("Out", {eltadd0_out->Name(), eltadd1_out->Name(), eltadd2_out->Name()});
  ...
  auto handler = [&](const GraphPatternDetector::subgraph_t& subgraph,
    ...
    std::unordered_set<const Node*> marked_nodes({// input0
