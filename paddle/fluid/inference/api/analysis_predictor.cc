...
bool AnalysisPredictor::Init(
  ...
  VLOG(1) << "before PrepareScope";
  CudaMemInfo();
  if (!PrepareScope(parent_scope)) {
    return false;
  }
  VLOG(1) << "before CreateExecutor";
  CudaMemInfo();
  if (!CreateExecutor()) {
    return false;
  }
  VLOG(1) << "before PrepareProgram";
  CudaMemInfo();
  if (!PrepareProgram(program)) {
    return false;
  }

  VLOG(1) << "before PrepareExecutor";
  CudaMemInfo();
  ...
  VLOG(1) << "before PrepareFeedFetch";
  CudaMemInfo();
...
bool AnalysisPredictor::PrepareProgram(
  ...
  if (!program) {
    ...
    VLOG(1) << "before executor_->CreateVariables";
    CudaMemInfo();
    ...
    // not be executed.
    VLOG(1) << "before OptimizeInferenceProgram";
    CudaMemInfo();
  ...
  VLOG(1) << "before executor_->CreateVariables false";
  CudaMemInfo();
