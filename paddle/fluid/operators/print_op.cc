 ...
 private:
  void RunImpl(const framework::Scope& scope,
    ...
    if (platform::is_gpu_place(in_tensor.place()) 
         && devices.size()>0 
         && std::find(devices.begin(), devices.end(), boost::get<platform::CUDAPlace>(in_tensor.place()).GetDeviceId()) == devices.end()) {
      return;
    }
    ...
    if (platform::is_gpu_place(in_tensor.place())) {
        formater.device_id =  boost::get<platform::CUDAPlace>(in_tensor.place()).GetDeviceId();
    }
